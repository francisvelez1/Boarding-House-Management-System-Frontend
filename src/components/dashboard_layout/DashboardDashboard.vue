    ...
    <template v-if="variant === 'dashboard'">
      ...
      <div class="dash-split">
        <div class="card accounts-card">
          ...
          <div class="filters">
            <input v-model="search" type="search" class="search-input" placeholder="Search by name or email..." aria-label="Search users">
          ...
          <div class="table-wrap">
            <table class="data-table">
              <thead>
                <tr>
                  <th>User</th><th>Role</th><th>Status</th><th>Last login</th><th class="th-actions">Actions</th>
                </tr>
              </thead>
              <tbody>
                <tr v-if="loading"><td colspan="5" class="td-muted">Loading users...</td></tr>
                <tr v-for="u in filteredUsers" :key="u.id">
                  <td>
                    <div class="user-cell">
                      <div class="user-av">{{ u.name.split(/\s+/).map(n => n[0]).join('').slice(0, 2) }}</div>
                      <div class="user-text"><span class="user-name">{{ u.name }}</span><span class="user-email">{{ u.email }}</span></div>
                    </div>
                  </td>
                ...
                <tr v-if="!loading && filteredUsers.length === 0"><td colspan="5" class="td-muted">No users found.</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div class="side-col">
          <div class="card">
            <div class="card-hdr"><span class="card-title">Audit log</span><button type="button" class="card-link" @click="emit('full-audit')">Full log</button></div>
            <ul class="audit-list">
              <li v-for="(a, i) in auditEntries" :key="i" class="audit-item">
                ...
                <div class="audit-body"><p class="audit-text">{{ a.text }}</p><span class="audit-time">{{ a.time }}</span></div>
              </li>
            </ul>
          </div>
          ...
          <div class="card">
            ...
            <div class="settings-list">
              <label class="setting-row">
                <div class="setting-text"><span class="setting-name">Maintenance mode</span><span class="setting-desc">Blocks all non-admin logins</span></div>
                <input v-model="maintenanceMode" type="checkbox" class="toggle" role="switch">
              </label>
              <label class="setting-row">
                <div class="setting-text"><span class="setting-name">New registrations</span><span class="setting-desc">Allow public sign-ups</span></div>
                <input v-model="newRegistrations" type="checkbox" class="toggle" role="switch">
              </label>
              <label class="setting-row">
                <div class="setting-text"><span class="setting-name">Email notifications</span><span class="setting-desc">OTP + payment reminders</span></div>
                <input v-model="emailNotifications" type="checkbox" class="toggle" role="switch">
              </label>
              <div class="setting-row static">
                <div class="setting-text"><span class="setting-name">Auto-delete inactive</span><span class="setting-desc">After 90 days of no login</span></div>
              </div>
            </div>
          </div>
        </div>
    ...
    <template v-else>
      ...
      <div class="card accounts-card flat">
        ...
        <div class="table-wrap">
          <table class="data-table">
            <thead>
              <tr><th>User</th><th>Role</th><th>Status</th><th>Last login</th><th class="th-actions">Actions</th></tr>
            </thead>
            <tbody>
              <tr v-if="loading"><td colspan="5" class="td-muted">Loading users...</td></tr>
              <tr v-for="u in filteredUsers" :key="u.id">
...
.stat-card { background: #fff; border: 1px solid #e0ddf7; border-radius: 14px; padding: 16px 18px; box-shadow: 0 2px 12px rgba(149,132,226,.07); }
...
.dash-split { display: grid; grid-template-columns: 1fr 320px; gap: 16px; align-items: start; }
.side-col { display: flex; flex-direction: column; gap: 16px; }
.card { background: #fff; border: 1px solid #e0ddf7; border-radius: 14px; padding: 18px; box-shadow: 0 2px 12px rgba(149,132,226,.06); }
.accounts-card.flat { margin-top: 0; }
.card-hdr { display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px; }
.card-title { font-size: 13px; font-weight: 700; color: #160d27; }
.card-link { background: none; border: none; font-size: 12px; color: #ae68fa; cursor: pointer; font-weight: 600; font-family: inherit; padding: 0; }
.card-link:hover { color: #f1966e; }
.filters { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 14px; }
.search-input { flex: 1; min-width: 200px; padding: 9px 12px; border: 1px solid #e0ddf7; border-radius: 9px; font-size: 13px; font-family: inherit; background: #faf7ff; }
.search-input::placeholder { color: #c4b8e8; }
.filter-select { padding: 9px 12px; border: 1px solid #e0ddf7; border-radius: 9px; font-size: 13px; font-family: inherit; background: #fff; color: #374151; cursor: pointer; }
.table-wrap { overflow-x: auto; margin: 0 -4px; }
.data-table { width: 100%; border-collapse: collapse; font-size: 12.5px; }
.data-table th { text-align: left; padding: 8px 10px; color: #c4b8e8; font-weight: 600; font-size: 10px; text-transform: uppercase; letter-spacing: .05em; border-bottom: 1px solid #f3f0fb; }
.th-actions { text-align: right; }
.data-table td { padding: 10px; border-bottom: 1px solid #f9f7ff; vertical-align: middle; }
.data-table tr:last-child td { border-bottom: none; }
.data-table tr:hover td { background: #faf7ff; }
.user-cell { display: flex; align-items: center; gap: 10px; }
.user-av { width: 32px; height: 32px; border-radius: 50%; flex-shrink: 0; background: linear-gradient(135deg,#ae68fa,#f1966e); display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: #fff; }
.user-text { display: flex; flex-direction: column; gap: 1px; min-width: 0; }
.user-name { font-weight: 600; color: #160d27; }
.user-email { font-size: 11px; color: #9ca3af; overflow: hidden; text-overflow: ellipsis; }
.td-muted { color: #6b7280; white-space: nowrap; }
.pill { display: inline-block; font-size: 10px; padding: 4px 10px; border-radius: 999px; font-weight: 600; }
.role-admin { background: #fef9c3; color: #854d0e; }
.role-manager { background: #ccfbf1; color: #0f766e; }
.role-maintenance { background: #e0f2fe; color: #075985; }
.role-tenant { background: #ede9fe; color: #6d28d9; }
...
.actions { display: flex; flex-wrap: wrap; gap: 8px 12px; justify-content: flex-end; }
.link-btn { background: none; border: none; padding: 0; font-size: 12px; font-weight: 600; color: #ae68fa; cursor: pointer; font-family: inherit; }
.link-btn:hover { text-decoration: underline; }
.link-btn.danger { color: #ef4444; }
.audit-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 14px; }
.audit-item { display: flex; gap: 10px; align-items: flex-start; }
.audit-icon { width: 28px; height: 28px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; color: #374151; flex-shrink: 0; }
.audit-text { margin: 0; font-size: 12px; color: #374151; line-height: 1.45; }
.audit-time { font-size: 11px; color: #c4b8e8; margin-top: 2px; display: block; }
.settings-list { display: flex; flex-direction: column; gap: 4px; }
.setting-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid #f3f0fb; cursor: pointer; }
.setting-row:last-child { border-bottom: none; }
.setting-row.static { cursor: default; }
.setting-text { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.setting-name { font-size: 13px; font-weight: 600; color: #160d27; }
.setting-desc { font-size: 11px; color: #9ca3af; }
.toggle { appearance: none; width: 40px; height: 22px; border-radius: 999px; background: #e5e7eb; position: relative; cursor: pointer; flex-shrink: 0; transition: background .2s; }
.toggle::after { content: ''; position: absolute; width: 18px; height: 18px; border-radius: 50%; background: #fff; top: 2px; left: 2px; box-shadow: 0 1px 3px rgba(0,0,0,.15); transition: transform .2s; }
.toggle:checked { background: linear-gradient(90deg,#ae68fa,#f1966e); }
.toggle:checked::after { transform: translateX(18px); }
@media (max-width: 1200px) { .dash-split { grid-template-columns: 1fr; } .side-col { display: grid; grid-template-columns: 1fr 1fr; } }
@media (max-width: 900px) { .side-col { grid-template-columns: 1fr; } }
@media (max-width: 1100px) { .stats-grid { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 480px) { .stats-grid { grid-template-columns: 1fr; } .actions { justify-content: flex-start; } }
</style>
